...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!_.isObjectLike(fields) || _.isArray(fields)) {
<span class="cstat-no" title="statement not covered" >    return Promise.reject(new Error('Update requires a hash of fields=&gt;values to update to'));</span>
  }
&nbsp;
  if (_.isEmpty(fields)) {
...
Table.prototype.save = function (record, options = {}) {
  if (!this.pk) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.is_insertable_into) {
      return Promise.reject(new Error(`${this.name} is not writable`));
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return Promise.reject(new Error(`${this.name} has no primary key, use insert or update to write to this table`));</span>
  } else <span class="missing-if-branch" title="if path not taken" >I</span>if (!_.isObjectLike(record) || _.isArray(record)) {
<span class="cstat-no" title="statement not covered" >    return Promise.reject(new Error('Must provide an object with all fields being modified and the primary key if updating'));</span>
  }
&nbsp;
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="https://istanbul.js.org/" target="_blank">istanbul</a> at Fri Apr 06 2018 19:37:51 GMT-0400 (EDT)
</div>
</div>
